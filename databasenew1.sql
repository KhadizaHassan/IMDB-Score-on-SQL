...
SELECT customerName, CreditLimit       ## Method 1 was Giving me this
FROM customers
WHERE country = 'USA' AND CreditLimit > 1000;


SELECT customerName, City, State, CreditLimit     ## So, I did this 
FROM customers
WHERE country = 'USA' AND CreditLimit > 1000 limit 10;
...
SELECT efullName, email FROM employees WHERE employeenumber IN (1370,1337,1702,1401);

SELECT * FROM employees;
select employeeNumber from employees;
select * from customers;
SELECT CONCAT(firstName, ' ', lastName) AS efullName
FROM employees;
ALTER TABLE employees ADD efullName varchar(150);

UPDATE employees
SET efullName = CONCAT(firstName, ' ', lastName)
WHERE employeeNumber between 1002 and 1625;
select lastName, firstName, efullName from employees;
...
-- SET efullName = CONCAT(firstName, ' ', lastName)
-- WHERE employeeNumber between 1002 and 1625;
-- select efullName from employees;

-- SELECT DISTINCT SalesRepresentative FROM Customers WHERE Country IN ('Spain', 'France', 'Italy'); ## send an error of 1054/ no column in the field list
-- SELECT employeeNumber
-- FROM employees
-- WHERE jobTitle = 'Sales Rep' and country='Spain' or country='France' or country='Italy';
...
-- SELECT e.employeefullName, c.customerName, c.city, c.state, c.country
-- FROM employees as e
-- INNER JOIN customers as c ON e.employeeNumber = c.salesRepEmployeeNumer;
...
SELECT e.jobTitle, o.city
FROM employees as e
INNER JOIN offices as o ON e.officeCode = o.officeCode WHERE o.officeCode = 7;

DELETE FROM offices WHERE jobTitle = 'Sales Representative' AND officeCode=7;
...
-- DELETE FROM offices WHERE jobTitle = 'Sales Representative' AND city='London';
-- SET foreign_key_checks=1;
-- select * from offices;
...
-- SELECT country FROM offices WHERE officeCode = 7;
-- DELETE FROM offices WHERE jobTitle = 'Sales Representative' AND officeCode=7;
-- SET foreign_key_checks=1;
-- select * from offices;
-- select jobTitle from employees;
...
-- VALUES ('joan', 'Cheai', '+819744323487', '3/8, Beach', 'London', '660220','Nouth',POINT(4,2));

-- DELETE FROM Employees
-- WHERE jobTitle = 'Sales Representative' AND OfficeLocation = 'London';
...
SELECT DISTINCT customerNumber, paymentDate, amount FROM payments WHERE paymentDate<"2004-10-28";
...
SELECT *          -- Method 1 by Chaining
FROM customers 
WHERE customerNumber in 
(select distinct customerNumber from payments where paymentDate BETWEEN '2003-04-01' AND '2004-03-31');



SELECT *           -- Method 2 by joining
FROM customers as c
JOIN payments as p
ON c.customerNumber = p.customerNumber;
SELECT * FROM customers WHERE country = 'USA' AND paymentDate BETWEEN '2003-04-01' AND '2004-03-31';


   -- ##Find the total number of payments made each customer before October 28, 2004.


select distinct customerNumber, count(*) as totalpaments from payments where paymentDate <'2004-10-28' group by customerNumber;   -- ## Method 1
SELECT customerNumber, COUNT(*) as totalPayments FROM payments WHERE paymentDate<"2004-10-28" GROUP BY customerNumber;

SELECT distinct c.customerNumber, COUNT(*) AS totalPayments     -- ## Method 2 by joining 
FROM Customers c
JOIN Payments p
ON c.customerNumber = p.customerNumber
WHERE p.paymentDate < '2004-10-28'
GROUP BY c.customerNumber;

-- Q: Find the total amount paid by each customer payment before October 28, 2004.

SELECT customerNumber, SUM(amount) as totalPayment    --  ## Method 1
   FROM payments WHERE paymentDate<"2004-10-28"
   GROUP BY customerNumber;
   
   
SELECT distinct c.customerNumber, sum(amount) AS totalPayments     -- ## Method 2 by joining 
FROM Customers c
JOIN Payments p
ON c.customerNumber = p.customerNumber
WHERE p.paymentDate < '2004-10-28'
...
select distinct customerNumber, sum(amount) as totalpayment FROM payments WHERE paymentDate<"2004-10-28" group by customerNumber;
...
select customerNumber, count(*) as totalpayments, sum(amount) as totalamounts from payments where paymentDate <'2004-10-28' group by customerNumber;
...
-- QUESTION: Display the full name of point of contact each customer in the United States in upper case,
--  along with their phone number, sorted by alphabetical order of customer name.

SELECT * FROM customers;        -- ## Method  by me

SELECT CONCAT(contactLastName, ' ', contactFirstName) AS ccfullName, phone
FROM customers where country = "USA";
ALTER TABLE customers ADD ccfullName varchar(150);



SELECT customerName,             -- ## Method 2 from sheet
...
SELECT customerName,
    LOWER(SUBSTRING(country, 1, 3)) AS countryCode
FROM Customers
ORDER BY customerName
...
-- QUESTION: Display the product code, product name, buy price, sale price and profit margin percentage ((MSRP - buyPrice)*100/buyPrice) for the 10 products with the highest profit margin. 
...
SELECT 
    customerNumber, MAX(amount) AS largestPayment
FROM
    payments
WHERE
...
-- QUESTION: For the above query, format the amount properly with a dollar symbol 
-- and comma separation (e.g $26,267.62),
 -- and also show the month as a string.
...
-- adress, phone from office.      for each employee by their e-id, or full name.   
  
-- select * from offices;

-- SELECT 
--     country,
--     CONCAT(addressLine1, '', addressLine2) AS fulladress
-- FROM
--     offices;
...
--  SELECT employeeNumber, 
--     CONCAT(lastName,"",firstName) AS employeefullName, jobTitle
-- FROM
--     employees;   
...
-- select * from orders;    -- select * from products;     -- select * from orderDetails;
SELECT 
    o.orderNumber, 
    o.customerNumber,
    o.OrderDate, 
...
SELECT 
    c.customerName, SUM(payments.amount)
FROM
    payments
        INNER JOIN
...
FROM
    ...
    Orders ON Customers.customerNumber = Orders.customerNumber
        INNER JOIN
    OrderDetails Detail ON Orders.orderNumber = Detail.orderNumber
...
-- 7. Who are the employees in Boston?
                         -- Two possible solutions, one using JOIN, the other one using a SUBQUERY.                -- JOIN solution
...
SELECT 
    ...
    orders.orderNumber,
    SUM(orderdetails.priceEach * orderdetails.quantityOrdered) AS tot_value
FROM
    customers
        JOIN
    orders ON customers.customerNumber = orders.customerNumber
...
SELECT DISTINCT
    ...
    2 * (products.buyPrice) AS twice_buy_price,
...
WHERE
    orderdetails.priceEach > 2 * products.buyPrice;
...
--             order_details od
--         WHERE
--             od.product_id = p.product_id) = (SELECT 
--             COUNT(DISTINCT order_id)
--         FROM
--             orders);
...
FROM
    ...
    orderdetails ON products.productCode = orderdetails.productCode
WHERE
    orderdetails.priceEach < (0.8 * products.MSRP)
...
SELECT 
    productName, orderDate, DAYNAME(orderDate) AS 'DayName'
FROM
    products
        INNER JOIN
    orderdetails ON products.productCode = orderdetails.productCode
...
WHERE
    DAYNAME(Orders.orderDate) = 'MONDAY';
...
FROM
    customers c
        JOIN
    employees e ON c.salesRepEmployeeNumber = e.reportsTo;
...
--     customers c
--         JOIN
--     employees e ON e.reportsTo = e.jobTitle;
...
-- For each product line, rank the products based on their MSRP 
-- (Manufacturer's Suggested Retail Price) from highest to lowest. Display the product name, 
-- its product line, its MSRP, and its rank within that specific product line

SELECT 
    productname,
    productLine,
    msrp,
    RANK() OVER (PARTITION BY productline ORDER BY msrp DESC) AS rankwithinline
FROM 
    products;
...
-- VALUES
  -- (8, 'Dhaka', '+88-415-555-1234', '100 Market Street', 'USA', '94105', 'NA'),
  -- (9, 'Joshore', '+88-3-1234-5678', '1 Chiyoda', 'Japan', '100-8211', 'APAC'),
  -- (10, 'Khulna', '+88-20-1234-5678', '200 Oxford Street', 'UK', 'W1D 1AB', 'EMEA');
  
  
-- > 2. Explore what happens if you don't provide a value for a column marked as `not null`.
-- 			When a column is marked as  in a database schema, 
-- 			it means that this column must always have a value—null or empty values are not allowed.
-- 			If you try to insert a record into a table and fail to provide a value for a  column, 
-- 			the database system will throw an error and reject the operation.

-- If you try to insert a record without providing values for  or , like this:
-- 		INSERT INTO Employees (LastName)
-- 		VALUES ('Smith');  without ID, or firstname


-- > 3. Try adding an entry with a primary key matching the an existing entry.
		-- The database will reject this operation, displaying an error similar to: "Duplicate entry for primary key".
-- 			If you attempt to add an entry to a table with a primary key that matches an existing entry, 
--             the database will throw an error because the primary key must always be unique. 


-- > 4. Retrieve and display just the city and phone number information for each office.
-- select city, phonenumber from offices;
-- 		SELECT City, PhoneNumber
-- 		FROM Offices;

-- > **EXERCISES** employees:
-- >
-- > 1. Try inserting an entry into `employees` with an invalid value for `officeCode`. 
-- insert into offices (officecodes) value 
-- (8)
-- (9)
-- (10);
			-- INSERT INTO offices (officeCode, city) VALUES 
			-- (1, 'New York'), (2, 'Los Angeles');

-- > 2. Try inserting an entry into `employees` with an invalid value for `reportsTo`.
-- INSERT INTO employees ( reportsTo) values
-- (lane)
-- (kane);   NOW, it will through an error because all the employee should have an employeeID
			-- INSERT INTO employees (employeeID, firstName, lastName, reportsTo)
			-- VALUES (101, 'Jane', 'Doe', 999);

-- > 3. Create a new office location and add some employees for the new location.
-- INSERT INTO Offices (officeCode, city, phoneNumber)
-- VALUES (3, 'Chicago', '312-555-1234');

-- INSERT INTO Employees (employeeID, firstName, lastName, officeCode)
-- VALUES (201, 'John', 'Smith', 3),
--        (202, 'Emily', 'Johnson', 3),
--        (203, 'Michael', 'Williams', 3);


-- > **EXERCISES** customers:

-- > 1. List the customers in the United States with a credit limit higher than \\$1000.
-- select * name from customers where country = 'USA' and creditlimit > 1000;
...
-- select * employeecode from customers where country = 'Spain', 'France', 'Italy' and jobtitle = 'representatives';

-- 			SELECT DISTINCT SalesRepresentative
-- 			FROM Customers
...
-- 			DELETE FROM Employees
-- 			WHERE Title = 'Sales Representative' AND OfficeLocation = 'London';
...
-- 			SELECT *
-- 			FROM Customers
-- 			WHERE Name LIKE '%Toys%';

-- > **EXERCISES**:

-- > 1. List the 5 most expensive products from the "Planes" product line
-- select * productline from products where productname = "Planes' limit 5;

-- 			SELECT ProductName, ProductLine, Price
-- 			FROM Products
...
-- select productname, stockquintity from products where stockquintity <100;
...
-- > **EXERCISE**: modify
-- Add, remove and modify one column in each of the tables created above. What happens when you rename or remove a column that is a primary key?
      -- To Add
--       ALTER TABLE Offices
...
-- select country, state, city from offices ordered by country = "usa"/or ordered by country DESC
-- 				SELECT Country, State, City, Address
-- 				FROM Offices
-- 				ORDER BY Country ASC, State ASC, City ASC;
-- 2. How many employees are there in the company?
-- select * employees from employeeid
-- 			SELECT COUNT(*) AS TotalEmployees
-- 			FROM Employees;

-- 3. What is the total of payments received?
-- select * paymenttotal from payments
...
-- select sum(paymenttotal) as totalpay from payments where paymentdate - '2004-10-28'
-- 			SELECT SUM(Amount) AS TotalPayments
-- 			FROM Payments
-- 			WHERE PaymentDate = '2004-10-28';

-- 6. Report those payments greater than \\$100,000.
-- select * paymenttotal from payments where paymenttoal >100,000
-- 				SELECT *
-- 				FROM Payments
-- 				WHERE Amount > 100000;

-- 7. List the products in each product line.
-- select * products from productlines
...
-- SELECT AVG(((MSRP - buyPrice) * 100 AS AverageMarkupPercentage from products
			-- 		SELECT AVG(((MSRP - buyPrice) / buyPrice) * 100) AS AverageMarkupPercentage
			-- 		FROM Products;

-- 12. How many distinct products does ClassicModels sell?
-- select distint(productname) as PN from  products
-- 				SELECT COUNT(DISTINCT ProductName) AS DistinctProductCount
-- 				FROM Products;

-- 13. Report the name and city of customers who don't have sales representatives?
-- select name, city from customers where SalesRepresentative = ' null'
-- 			SELECT Name, City
-- 			FROM Customers
-- 			WHERE SalesRepresentative IS NULL;

-- 14. What are the names of executives with VP or Manager in their title? Use the CONCAT function to combine the employee's first name and last name into a single field for reporting.
-- select concat(firstname, ' ' lastname) as fullname from employees where jobtitle like '%VIP%' or jobtitle like '%Manager%';
-- 				SELECT CONCAT(FirstName, ' ', LastName) AS FullName
-- 				FROM Employees
-- 				WHERE Title LIKE '%VP%' OR Title LIKE '%Manager%';

-- 15. Which orders have a value greater than $5,000?
-- select * invoice from oders where ordertotal > 5000;
-- 				SELECT *
-- 				FROM Orders
-- 				WHERE OrderValue > 5000;

                          -- windows function 

 -- Q: Show the list of all the payments made by the customers of Classic Models, and assign a unique payment number to each payment
SELECT *,
  ROW_NUMBER()
  OVER ()
  AS paymentNumber
...
-- QUESTION : Express each payment made by a customer as a fraction of the total payment made by the customer. Show the checkNumber, customerNumber, paymentDate, amount & fractionOfTotal
SELECT *, amount/ (
   SUM(amount) OVER
...
-- QUESTION : Show the running total payments received by Classic Models, starting from the oldest to the newest. Show the checkNumber, customerNumber, paymentDate, amount & runningTotal
...
-- QUESTION : Show the running total and running average of payments by each customer, starting from the oldest to the newest. Also output the customerNumber, and paymentDate.
...
-- QUESTION : Determine the team size for each employee i.e. how many people report to the same manager as the employee. Output employeeNumber, lastName, firstName, reportsTo, and teamSize.
...
-- QUESTION : For each order, show the order number, total order amount and the difference between the order amount and the maximum/minimum order amount for the customer.
...
-- QUESTION : Provide serial numbers for the customers of each sales representative, sorted by dates of each customer's earliest orders
...
-- QUESTION : Rank the offices of Classic Models based on the number of employees (lowest first). Compare the outputs of ROW_NUMBER,RANK, DENSE_RANK and CUME_DIST.
...
FROM (SELECT o.officeCode, city, COUNT(*) AS employeeCount
  FROM offices o JOIN employees e
  ON o.officeCode = e.officeCode
  GROUP BY o.officeCode, city) AS oe;
  
 -- QUESTION : Compute the quartile and percentile for each payment made by a customer. Output checkNumber, paymentDate, amount, customerNumber, quartile, and percentRank.
...
-- QUESTION : For each order, show the number of days elapsed since the last order placed by the same customer. Output OrderNumber, orderDate, customerNumber, and daysSinceLastOrder.
...
-- QUESTION : List order of each customer sorted by order amount (highest to lowest) and display the difference in amount between each order and the next highest order by the same customer.
...
--   FROM orders o JOIN orderdetails od
--   ON o.orderNumber = od.orderNumber
--   GROUP BY o.orderNumber, o.customerNumber AS orderswithValues;  (showing error here)
  
--  QUESTION </b>: For each payment made by a customer compute the difference between the payment amount and the first/second/last payment made by the customer.
...
--   ROWS BETWEEN UNBOUNDED PRECEDING        -- ( showing error here)
